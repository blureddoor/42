...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/12/20 20:05:14 by lvintila          #+#    #+#             */
/*   Updated: 2021/12/20 20:33:20 by lvintila         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char *_getenv(char *name, char **env)
{
	...
	while (env[i] && env)
	{
		current_env = NULL;
		token2 = NULL;
		current_env = ft_strdup(env[i]);
		token1 = ft_strtok(current_env, "=");
		token2 = ft_strdup(strtok(NULL, "="));
		if (ft_strcmp(env_name, token1) == 0)
		{
			break;
		}
		free(current_env);
