...
/*   Updated: 2020/09/16 19:49:22 by mvillaes         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
#define ANCHO_DIBUJO	20
#define ALTO_DIBUJO		20
...
int		main(void)
{
	...
	while (++count_h < ALTO_DIBUJO)
	{
		...
		{
			if (count_w % 3)
				img.data[count_h * ANCHO_DIBUJO + count_w] = 0xFFFFFF;
			else
				img.data[count_h * ANCHO_DIBUJO + count_w] = 0xFF0000;
		}
	}
	mlx_put_image_to_window(mlx->mlx_ptr, mlx->mlx_win, fondo, 0, 0);
	mlx_put_image_to_window(mlx->mlx_ptr, mlx->mlx_win, img.img_ptr, 0, 0);
